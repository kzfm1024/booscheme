...
def repl2(prompt='lispy> ', inport=InPort(sys.stdin), out=sys.stdout):
    "A prompt-read-eval-print loop."
    def cc_print(val):
        if val is not None and out: print >> out, to_string(val)
    global global_env
    sys.stderr.write("Lispy version 2.0\n")
    sys.stderr.write("repl2\n")
    while True:
        try:
            if prompt: sys.stderr.write(prompt)
            x = parse(inport)
            if x is eof_object: return
            eval2(x, global_env, cc_print)
        except Exception as e:
            print '%s: %s' % (type(e).__name__, e)
...
def eval2(x, env, cc):
    ...
    while True:
        if isa(x, Symbol):       # variable reference
            ...
            (_, test, conseq, alt) = x
            def cc_if(test):
                if test: return conseq
                else: return alt
            x = eval2(test, env, cc_if)
        elif x[0] is _set:       # (set! var exp)
            (_, var, exp) = x
            def cc_set(x): env.find(var)[var] = x
            eval2(exp, env, cc_set)
            return cc(None)
        elif x[0] is _define:    # (define var exp)
            (_, var, exp) = x
            def cc_define(x): env[var] = x
            eval2(exp, env, cc_define)
            return cc(None)
#        elif x[0] is _lambda:    # (lambda (var*) exp)
#            (_, vars, exp) = x
#            return cc(Procedure(vars, exp, env))
#        elif x[0] is _begin:     # (begin exp+)
#            for exp in x[1:-1]:
#                eval(exp, env)
#            x = x[-1]
